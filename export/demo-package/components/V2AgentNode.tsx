...
export const V2AgentNode: React.FC<NodeProps<V2AgentNodeData>> = ({ data, id, selected }) => {
  ...
  // Use instance name if available, otherwise use prototype name
  const displayName = agentInstance?.name || agent.name;
  ...
  // Design store for node updates
  const { updateNode, deleteNode } = useDesignStore();
  ...
  const handleToggleMinimize = () => {
    updateNode(id, {
      data: { ...data, isMinimized: !isMinimized }
    });
  };
  ...
  const handleEdit = () => {
    ...
    if (navigationHandler && agent) {
      ...
      const { selectAgent } = useDesignStore.getState();
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    // Get LLM config
    const llmConfigs = useRuntimeStore.getState().llmConfigs;
          ...
          <div className="mb-2 p-2 bg-gray-800 rounded-lg border border-gray-600">
            <div className="flex items-center mb-1">
              ...
              <span className="text-xs font-semibold text-gray-300">
                {isError ? 'Erreur outil' : 'Résultat outil'}: {message.toolName}
              </span>
            </div>
                ...
                {message.toolCalls.map((call) => (
                  <div key={call.id} className="flex items-center">
                    <p className={`text-xs font-semibold text-gray-400 flex items-center ${
                      message.status === 'executing_tool' ? 'animate-pulse' : ''
                    }`}>
                      <ToolIcon className={`w-3 h-3 mr-1 ${
                        message.status === 'executing_tool' ? 'animate-spin' : ''
                      }`} />
                      {message.status === 'executing_tool' 
                        ? `Exécution de ${call.name}...` 
                        : 'Outil appelé'
                      }
                    </p>
                    {message.status !== 'executing_tool' && (
    ...
    `}>
      ...
      <div className="flex items-center justify-between p-3 border-b border-gray-700/80 
        ...
        <div className="flex items-center space-x-2">
          <div className={`w-2 h-2 rounded-full shadow-lg transition-all duration-200 ${
            isLoading 
            ...
            {messages.length === 0 && (
              <div className="text-center text-gray-500 text-sm" style={{ userSelect: 'text' }}>
                <div className="bg-gray-800/40 border border-gray-700/50 rounded-lg p-4
                                backdrop-blur-sm">
                  <p className="text-gray-400">Conversation vide</p>
                  <p className="text-xs mt-1 text-gray-500">Tapez un message pour commencer</p>
                ...
                <div className="flex flex-col space-y-1">
                  ...
                  <Button
                    ...
                    className={`p-2 h-8 w-8 transition-all duration-200 rounded-md
                                hover:scale-110 active:scale-95 ${
                      isLoading || (!userInput.trim() && !attachedFile)
